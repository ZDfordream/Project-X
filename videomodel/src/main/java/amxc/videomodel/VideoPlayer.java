...
import amxc.videomodel.utils.LightUtil;
import amxc.videomodel.utils.PlayerUtil;
import llf.videomodel.R;

import static amxc.videomodel.utils.PlayerUtil.dip2px;
...
public class VideoPlayer extends FrameLayout implements View.OnClickListener,
        ...
        if (autoPlay) {
            ...
            if (!PlayerUtil.isNetworkConnected(context) && videoPath.startsWith("http")) {
                ...
                if (PlayerUtil.isMobileConnected(context)) {
                    PlayerUtil.showToast(context,"请注意,当前网络状态切换为3G/4G网络");
                }
                //添加播放路径
                try {
                    if(videoPath==null){
    ...
    public void playVideo(String url, String title, int position) {
        ...
        if (TextUtils.isEmpty(url)) {
            PlayerUtil.showToast(context, "播放地址为空");
            return;
        }
        //销毁ControllerView
        destroyControllerTask(true);
    ...
    private void resetMediaPlayer() {
        try {
            ...
            PlayerUtil.showToast(context, "播放器初始化失败");
        }
    }
